...
class PointKDTree : private Noncopyable
{
  ...
  public:
    Real minDistance; // min distance between points in leaf node
    Real maxDistance; // max distance between points in leaf node
    ...
    Node * GenKDTreeRec ( std::vector<const Point *> const & points );
    ...
    void rangeQueryRec(PointKDTree::Node * node, AxisAlignedBox3 const & query, std::vector<const Point *> & points_in_range) const;
